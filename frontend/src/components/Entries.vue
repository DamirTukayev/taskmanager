<template>
  <v-container>
    ...
    <v-btn color="primary" @click="dialog = true">Добавить запись</v-btn>

    <!-- Диалог с формой для добавления или редактирования задачи -->
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        <v-card-title class="headline">{{ isEditing ? 'Редактировать запись' : 'Добавить новую запись' }}</v-card-title>
        <v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="isEditing ? updateTask() : createTask()">{{ isEditing ? 'Изменить запись' : 'Добавить запись' }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    ...
    async updateTask() {
      const url = `http://localhost:8080/entries/${this.currentTaskId}`;
      try {
        const response = await fetch(url, {
          method: "PUT",
          headers: {
    ...
    async deleteTask(taskId) {
      const url = `http://localhost:8080/entries/${taskId}`;
      try {
        const response = await fetch(url, {
          method: "DELETE",
        });
    ...
    async toggleTaskStatus(task) {
      const url = `http://localhost:8080/entries/${task.id}`;
      const updatedTask = {
        ...task,
        status: task.status,
      };
